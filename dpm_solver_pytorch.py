...
def model_wrapper(
    ...
    def noise_pred_fn(x, t_continuous, cond=None, p_model_kwargs={}):
        t_input = get_model_input_time(t_continuous)
        if cond is None:
            output = model(x, t_input, **model_kwargs, **p_model_kwargs)
        else:
            output = model(x, t_input, cond, **model_kwargs, **p_model_kwargs)
        if model_type == "noise":
            return output
    ...
    def model_fn(x, t_continuous, p_model_kwargs={}):
        """
        The noise predicition model function that is used for DPM-Solver.
        """
        if guidance_type == "uncond":
            return noise_pred_fn(x, t_continuous, p_model_kwargs=p_model_kwargs)
        elif guidance_type == "classifier":
            assert classifier_fn is not None
            t_input = get_model_input_time(t_continuous)
            cond_grad = cond_grad_fn(x, t_input)
            sigma_t = noise_schedule.marginal_std(t_continuous)
            noise = noise_pred_fn(x, t_continuous, p_model_kwargs=p_model_kwargs)
            return noise - guidance_scale * sigma_t * cond_grad
        elif guidance_type == "classifier-free":
            if guidance_scale == 1. or unconditional_condition is None:
                return noise_pred_fn(x, t_continuous, cond=condition, p_model_kwargs=p_model_kwargs)
            else:
                x_in = torch.cat([x] * 2)
